...
export const translateFontVariantId = (googleFont: GoogleFont, fontName: FontName, fontWeight: number) => {
  ...
  const variantWithWeight = googleFont.variants?.find((variant) => variant === `${fontWeight.toString()}${italic}`);
