...
export const transformVectorPathsAsContent = (node: StarNode | LineNode | PolygonNode, baseX: number, baseY: number): PathAttributes => {
...
export const transformVectorPaths = (node: VectorNode, baseX: number, baseY: number): PathShape[] => {
  const pathShapes = node.vectorPaths
    .filter((vectorPath, index) => {
      return nodeHasFills(node, vectorPath, (node.vectorNetwork.regions ?? [])[index]) || node.strokes.length > 0;
    })
    .map((vectorPath, index) => transformVectorPath(node, vectorPath, (node.vectorNetwork.regions ?? [])[index], baseX, baseY));

  const geometryShapes = node.fillGeometry
    .filter((geometry) => !node.vectorPaths.find((vectorPath) => normalizePath(vectorPath.data) === normalizePath(geometry.data)))
...
const nodeHasFills = (node: VectorNode, vectorPath: VectorPath, vectorRegion: VectorRegion | undefined): boolean => {
...
const transformVectorPath = (
  ...
  return {
    ...
    fills: vectorPath.windingRule === 'NONE' ? [] : translateFills(vectorRegion?.fills ?? node.fills),
    svgAttrs: {
